...
router.get("/", requireAuth, requirePermission("laporan-resmi", "READ"), validate(laporanBkkSchema, "query"), async (req, res) => {
  try {
...
router.get("/pdf", requireAuth, requirePermission("laporan-resmi", "EXPORT"), validate(laporanBkkSchema, "query"), async (req, res) => {
  let browser;
  try {
    const { periodeId } = req.query;
    const data = await getBkkData(periodeId);

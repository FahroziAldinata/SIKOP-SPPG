...
router.get("/", requireAuth, requirePermission("laporan-resmi", "READ"), validate(laporanAnggaranSchema, "query"), async (req, res) => {
  try {
...
router.get("/pdf", requireAuth, requirePermission("laporan-resmi", "EXPORT"), validate(laporanAnggaranSchema, "query"), async (req, res) => {
  let browser;
  try {
    const { periodeId } = req.query;

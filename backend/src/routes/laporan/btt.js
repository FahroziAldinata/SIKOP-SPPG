...
router.get("/", requireAuth, requirePermission("laporan-resmi", "READ"), validate(laporanBttSchema, "query"), async (req, res) => {
  try {
...
router.get("/pdf", requireAuth, requirePermission("laporan-resmi", "EXPORT"), validate(laporanBttSchema, "query"), async (req, res) => {
  let browser;
  try {
    const { periodeId, kategori } = req.query;

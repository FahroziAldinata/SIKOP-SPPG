...
router.get("/", requireAuth, requirePermission("laporan-resmi", "READ"), validate(laporanLpaSchema, "query"), async (req, res) => {
  try {
...
router.get("/pdf", requireAuth, requirePermission("laporan-resmi", "EXPORT"), validate(laporanLpaSchema, "query"), async (req, res) => {
  let browser;
  try {
    const { periodeId, nomorDokumen, isLr } = req.query;
    const isLrBool = isLr === 'true';

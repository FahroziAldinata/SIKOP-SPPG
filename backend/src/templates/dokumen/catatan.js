/**
 * HTML template untuk Catatan Pengeluaran Bulanan (Catatan).
 * Field mapping:
...
/**
 * @param {object} data - data untuk Catatan
 * @returns {string} HTML string
 */
function renderCatatanHtml(data) {
...
<head>
  <meta charset="UTF-8">
  <title>Catatan Pengeluaran Bulanan — ${escapeHtml(namaLembaga)}</title>
  <style>
    ${SHARED_CSS}
    /* Catatan-specific styling */
    .tabel-ringkasan-catatan td {
...
<body>
  ...
  <h2 class="judul-dok" style="margin-top: 10px; text-align: center;">CATATAN PENGELUARAN BULANAN</h2>
  <div style="text-align: center; font-size: 11pt; margin-bottom: 15px; font-weight: bold;">
    ${escapeHtml(formattedPeriode)}
  </div>

  <table class="tabel-ringkasan-catatan" style="width: 100%; border: 1px solid #000; border-collapse: collapse; margin-bottom: 20px; font-size: 11pt;">
    <tbody>
      <tr>
      ...
      ${biayaLainnya > 0 ? `
      <tr>
        <td>Biaya Lainnya / Non-kategori</td>
        <td style="text-align: right;">${formatNumberTabel(biayaLainnya)}</td>
      </tr>
      ` : ''}
      <tr style="font-weight: bold;">
  ...
  <table class="tabel-transaksi-catatan" style="width: 100%; border: 1px solid #000; border-collapse: collapse; font-size: 10pt; page-break-inside: auto;">
    <thead>
      <tr style="background-color: #eaeaea; border-bottom: 1px solid #000;">
        ...
        <th style="border: 1px solid #000; padding: 6px; text-align: center; width: 130px;">Jumlah (Rp)</th>
      </tr>
...
module.exports = { renderCatatanHtml };

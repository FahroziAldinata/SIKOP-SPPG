...
- Tag `v2.0.0` dibuat Rozi (annotated `e42e051f` → `8fdbe8d`, pushed).

## Backlog Terbuka (urutan prioritas rekomendasi — keputusan mulai tetap milik Rozi)
...
- **GF-013**: test suite jalan di DB dev yang SAMA dengan development/manual-use (bukan DB test terpisah). Mitigasi parsial: backup/restore SystemConfig di afterAll chat.test.js. Keputusan Rozi: KNOWN RISK, jangan fix sekarang.
...
- **Known risk lama**: `bukti-lpd2m` DELETE — `fs.unlinkSync` file fisik di luar `$transaction` (best-effort; tx rollback bisa sisakan file fisik tanpa record). Keputusan Rozi: jangan diperbaiki sekarang.
...
- Null-file (sampah). `nul` sudah bersih. `.agents/` (RTK rules antigravity) untracked — keputusan Rozi: biarkan.

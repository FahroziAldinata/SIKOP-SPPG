...
- **Temuan 2 — Password DB campur** ✅ **RESOLVED 2026-08-12 (investigasi T2) — penjelasan resmi**:
  ...
  - **Risiko nyata 1 (rendah, dev-only, JANGAN di-fix di task ini)**: test tulis ke user seed nyata (username hardcoded) + restore cost 10 (di bawah kebijakan cost 12). Refactor test-user terpisah dijadwalkan — 1 task gabungan dengan GF-013 (akar sama: test tidak terisolasi dari data kerja nyata), keputusan Rozi 2026-08-12. Endpoint reset password AMAN (requirePermission admin-user + invalidasi sesi + audit + cost 12) — tanpa celah eksploitasi.

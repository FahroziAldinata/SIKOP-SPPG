...
### [2026-08-06] - Penyempitan Akses RBAC Final (resource granular) — hasil review business workflow
- **Keputusan**: (1) Resource `akuntan-akun` + `akuntan-jenis-pekerjaan` terpisah dari `akuntan-master` — AKUNTAN aksi penuh (sesuai implementasi lama), KEPALA_SPPG READ, **MITRA TIDAK diberi akses** (sebelumnya ikut `akuntan-master READ` → bisa lihat akun & jenis-pekerjaan, padahal dulu 403). (2) Resource `gizi-target` terpisah dari `gizi-master` — AHLI_GIZI READ/UPDATE + KEPALA_SPPG READ, **AKUNTAN & ASLAP TIDAK diberi akses** (dulu ikut `gizi-master READ` → bisa lihat target gizi, padahal dulu 403). (3) Resource `aslap-periode` granular — MITRA kembali bisa GET /api/aslap/periode (dulu requireRole ASLAP,MITRA,...) TANPA membuka sekolah/posyandu. (4) Resource `aslap-po-approval` — approval PO khusus ASLAP; KEPALA_SPPG TIDAK boleh (perilaku lama `requireRole("ASLAP")`).
- **Alasan**: Audit role lama vs efektif baru menemukan 5 endpoint READ akuntan-master melebar ke KEPALA_SPPG+MITRA dan GET master-target gizi terbuka ke AKUNTAN+ASLAP — bertentangan dengan business workflow role. Resource granular mengembalikan akses persis perilaku lama (git show origin/main sebagai sumber kebenaran).
- **Catatan**: Resource RBAC total 20 → 23 (aslap-periode, aslap-po-approval, akuntan-akun, akuntan-jenis-pekerjaan, gizi-target). Implementasi commit `c68aee4` + `c20a864`. Full suite 590/590 PASS.
- **Status**: IMPLEMENTED + MERGED + PUSHED (sesi 42, 2026-08-06).

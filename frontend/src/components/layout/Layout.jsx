...
export const Layout = () => {
  ...
  const fetchNotifikasi = async () => {
    if (!user || (user.role !== 'AKUNTAN' && user.role !== 'AHLI_GIZI' && user.role !== 'KEPALA_SPPG' && user.role !== 'ADMIN' && user.role !== 'MITRA')) return;
    setLoading(true);
    try {
  ...
  useEffect(() => {
    if (user?.role === 'AKUNTAN' || user?.role === 'AHLI_GIZI' || user?.role === 'KEPALA_SPPG' || user?.role === 'ADMIN' || user?.role === 'MITRA') {
      fetchNotifikasi();
  ...
  const PAGE_TITLES = {
    ...
    '/akuntan/laporan/harian': 'Laporan Harian',
    '/akuntan/laporan/lra': 'Laporan LRA',
    '/akuntan/laporan/lpd2m': 'Laporan LPD2M',
    '/akuntan/laporan/bapsd': 'Laporan BAPSD',
    '/akuntan/laporan/sptj': 'Laporan SPTJ',
    '/akuntan/laporan/btt': 'Laporan BTT',
      ...
      <div style={{
        ...
        <header className="glass-panel" style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            {(user?.role === 'AKUNTAN' || user?.role === 'AHLI_GIZI' || user?.role === 'KEPALA_SPPG' || user?.role === 'ADMIN' || user?.role === 'MITRA') && (

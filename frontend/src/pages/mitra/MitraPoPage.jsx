...
export const MitraPoPage = () => {
    ...
    const fetchPemeriksaanBahanPdf = async (poId, supplierNama) => {
        setPdfLoading(true);
        setPdfModalTitle(supplierNama ? `Pemeriksaan Bahan — ${supplierNama}` : 'Dokumen Pemeriksaan Bahan Makanan');
        setPdfModalOpen(true);
        try {
                                        ...
                                        <div key={po.id} style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', marginTop: '10px', overflow: 'hidden' }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 14px', backgroundColor: 'var(--bg)', fontSize: '12px', borderBottom: '1px solid var(--border)' }}>
                                                ...
                                                <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                                                    ...
                                                    {(po.status === 'DIREALISASI' || po.status === 'DITERIMA') && (
                                                        <button onClick={() => fetchPemeriksaanBahanPdf(po.id, po.supplier?.nama)} style={{ padding: '3px 8px', backgroundColor: '#17a2b8', color: '#fff', border: 'none', cursor: 'pointer', borderRadius: 'var(--radius-sm)', fontSize: '11px', fontWeight: 'bold' }}>Pemeriksaan Bahan</button>
                                                    )}
                                                </div>
                                            </div>
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div style={{ marginTop: '20px', display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
                            {(detailPoData.status === 'DIREALISASI' || detailPoData.status === 'DITERIMA') && (
                                <button
                                    onClick={() => fetchPemeriksaanBahanPdf(detailPoData.id, detailPoData.supplier?.nama)}
                                    style={{ padding: '8px 16px', backgroundColor: '#17a2b8', color: '#fff', border: 'none', cursor: 'pointer', borderRadius: 'var(--radius-sm)', fontSize: '13px', fontWeight: 'bold' }}
                                >
                                    Pemeriksaan Bahan
                                </button>
                            )}

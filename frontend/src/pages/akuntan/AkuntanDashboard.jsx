  ...
  const loadStats = async (pid) => {
    try {
      ...
      if (resBku.ok) {
        ...
        const transaksi = bkuData.data?.transaksi || [];
        if (transaksi.length > 0) {
          cashVal = transaksi[transaksi.length - 1].saldoBerjalan;
        }
      }

      const rabData = resRab.ok ? await resRab.json() : [];
      const resJurnalJson = resJurnal.ok ? await resJurnal.json() : [];
      const journalTotal = Array.isArray(resJurnalJson) 
        ? resJurnalJson.length 
        : (resJurnalJson.pagination?.total ?? resJurnalJson.data?.length ?? 0);

      setStats({
        totalKas: cashVal,
        rabCount: Array.isArray(rabData) ? rabData.length : (rabData.data?.length || 0),
